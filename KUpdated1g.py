...
def adipose(oxygen6, co6, k6, dnp6):
    ...
    if oxygen6 < 3*adiposeFrac * oxygen5init:
        co9 = co6 + adiposeFrac * oxygenAvail * rq * 8
    else:
        co9 = co6 + oxygen6 * rq
    k9 = k6
    dnp9 = dnp6
...
def skmc(oxygen7, co7, k7, dnp7):
    ...
    if oxygen7 < 3*skmcFrac * oxygen5init:
        co10 = co7 + skmcFrac * oxygenAvail * rq * 8 / 0.75
    else:
...
def rob(oxygen8, co8, k8, dnp8):
    ...
    if oxygen8 < 3*robFrac * oxygen5init:
        co11 = co8 + robFrac * oxygenAvail * rq * 8
    else:
        co11 = co8 + oxygen8 * rq
    k11 = k8
    dnp11 = dnp8 - 6.593e-4
...
airVol = 1800 # liters of air per hour
...
# plt.title('Venous Oxygen Over Time (1g DNP)')
# plt.xlabel('Time (hr)')
# plt.ylabel('Molar Flow Rate (moles/hr)')
# plt.savefig(f"1g-oxygen{stream}.png")

# plt.figure()
# plt.plot(data[f'co{stream}'])
# plt.title('Venous Carbon Dioxide Over Time (1g DNP)')
# plt.xlabel('Time (hr)')
# plt.ylabel('Molar Flow Rate (moles/hr)')
# plt.savefig(f"1g-co{stream}.png")

# plt.figure()
# plt.plot(data[f'k{stream}'])
# plt.title('Venous Potassium Over Time (1g DNP)')
# plt.xlabel('Time (hr)')
# plt.ylabel('Molar Flow Rate (moles/hr)')
# plt.savefig(f"1g-k{stream}.png")

# plt.figure()
# plt.plot(data[f'dnp{stream}'])
# plt.title('Venous DNP Over Time (1g DNP)')
# plt.xlabel('Time (hr)')
# plt.ylabel('Molar Flow Rate (moles/hr)')
# plt.savefig(f"1g-dnp{stream}.png")

# fig, axes = plt.subplots(2, 2, figsize=(15, 8))

# ax = axes[0,0]
# ax.plot(data[f'oxygen{stream}'])
# ax.set_title('Venous Oxygen Over Time (1g DNP)')
...
# ax.set_title('Venous Carbon Dioxide Over Time (1g DNP)')
# ax.set_xlabel('Time (hr)')
# ax.set_ylabel('Molar Flow Rate (moles/hr)')
# ax.grid(True)
...
# ax.set_title('Venous Potassium Over Time (1g DNP)')
# ax.set_xlabel('Time (hr)')
# ax.set_ylabel('Molar Flow Rate (moles/hr)')
# ax.grid(True)
...
# ax.set_title('Venous DNP Over Time (1g DNP)')
# ax.set_xlabel('Time (hr)')
# ax.set_ylabel('Molar Flow Rate (moles/hr)')
# ax.grid(True)
